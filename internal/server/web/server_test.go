...
func TestServer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err = NewEchoServer(store, WithKey([]byte("test")), WithPprof(true), WithCryptoKey("bla"))
			require.Error(t, err)
			_, err = NewEchoServer(store, WithKey([]byte("test")), WithPprof(true), WithCryptoKey(pemFile.Name()))
			require.Error(t, err)
			...
			require.NoError(t, err)
			go s.Start("127.0.0.1:12328")
			resp, err := http.Get("http://127.0.0.1:12328/ping")
			require.NoError(t, err)
			require.Equal(t, resp.StatusCode, http.StatusOK)
			require.NoError(t, resp.Body.Close())
			resp, err = http.Post("http://127.0.0.1:12328/value", "aplication/html", nil)
			require.NoError(t, err)
			require.NoError(t, resp.Body.Close())
			resp, err = http.Post("http://127.0.0.1:12328/update/", "application/json", bytes.NewBufferString(`{"value": 123}`))
			require.NoError(t, err)
			...
			////
			s, err = NewEchoServer(store, WithKey([]byte("test")), WithPprof(true), WithCryptoKey(""))
			require.NoError(t, err)
			req, err = http.NewRequest("GET", "/ping", nil)
			require.NoError(t, err)
			rr = httptest.NewRecorder()
